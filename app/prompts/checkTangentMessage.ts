export const checkTangentMessagePromptContent = (patientResponse, therapistPrompt) => { const content = 
    `Here is my problem: Is there a reasonable probability that this response indicates a desire to return to the cognitive rehabilitation session? <response>${patientResponse}</response> <prompt>${therapistPrompt}</prompt>. 
